...
async function boot() {
  ...
  try {
    ...
    if (err && err.status === 401) {
      ...
      if (sub) sub.textContent = 'Precisamos da sua conta do Mercado Livre conectada para listar seus anúncios.';
      const rc = document.getElementById('resultsContainer');
      if (rc) rc.style.display = 'none';
      renderError(err);                    // tela "conta desconectada" + botão Reconectar
      return;
    }
