...
const CONFIG = {
  ...
  USE_Q_PARAM: true,
  // Busca por SKU — a ML tem DOIS campos de SKU e um param para cada (doc oficial
  // "Busca de itens > Obter itens da conta de um vendedor > Por SKU"):
  //   seller_sku= -> atributo SELLER_SKU        (o que a conta 649733403 usa)
  //   sku=        -> campo seller_custom_field  (SKU do próprio item, estilo antigo)
  // O vendedor não sabe (nem precisa saber) em qual dos dois o SKU dele está, então
  // a busca tenta SELLER_SKU e, sem resultado, repete no seller_custom_field.
  // Validado com token real (03/08/2026): seller_sku=panela123 -> MLB1669552989;
  // sku=panela123 -> 0 (a conta não usa seller_custom_field em nenhum item).
  // Casamento EXATO e case-sensitive ("PANELA123" e "panela" devolvem 0) e combina
  ...
  SCAN_CONCURRENCY: 4,         // chamadas em voo na varredura de conta inteira (recortes)
  ...
  FICHA_TEXTO_MAX: 46,       // teto do texto do badge (a coluna de sinais tem largura fixa)
...
const state = {
  ...
  scan: null,            // recorte de conta inteira: { chave, itens, varridos, total, truncado }
  scanAtivo: false,      // true quando a página exibida veio do scan (pager e textos mudam)
  carga: 0,              // geração da carga: resposta de carga antiga não pisa na atual
};
...
async function loadAttrNames() {
  ...
  // Cache de sessão custa zero chamada: vale pra TODAS as categorias da página.
  // (O teto abaixo é de rede — cortar o cache junto deixava anúncio com o nome do
  // campo já guardado exibindo o texto genérico "faltam N campos".)
  const pendentes = [];
  cats.forEach((cat) => {
    const emCache = getCachedJson('mf_sel_attrs_' + cat);
    if (emCache && emCache.nomes) { state.attrNames[cat] = emCache.nomes; mudou = true; }
    else pendentes.push(cat);
  });
  if (mudou) renderCurrentRows();

  // O que falta buscar vai em lotes pequenos e paralelos: uma página de 50 anúncios
  // passa de 15 categorias distintas com facilidade, e em fila isso levava segundos.
  const aBuscar = pendentes.slice(0, CONFIG.FICHA_CATEGORIAS_CAP);
  for (let i = 0; i < aBuscar.length; i += CONFIG.FICHA_CATEGORIAS_LOTE) {
    const lote = aBuscar.slice(i, i + CONFIG.FICHA_CATEGORIAS_LOTE);
    await Promise.all(lote.map(async (cat) => {
      try {
        ...
        setCachedJson('mf_sel_attrs_' + cat, { nomes: nomes });
...
/* =========================================================================
   Recortes de conta inteira — "Com desconto" e "Frete grátis abaixo do piso"
   A ML não filtra nenhum dos dois no servidor (desconto é original_price > price e
   `shipping_cost=free` é ignorado no users/items/search — validado): o dado só existe
   no multiget. Então, com um desses recortes ligado, o painel varre a lista inteira
   (respeitando status, busca e chip ativos), guarda o resultado em memória e pagina
   em cima dele. Antes o recorte olhava só os 50 anúncios da página aberta, e o que
   estava com desconto na página 3 simplesmente não aparecia.
   Custo: uma varredura por combinação de filtros, reaproveitada até algo mudar.
   ========================================================================= */
function quickFilterAtivo() { return !!(state.discountOnly || state.freeShipUnder); }
// Frete grátis a ML FILTRA no servidor (labels=with_free_shipping) e ordena por preço.
// Então esse recorte não precisa de varredura: pede a lista já filtrada, do mais barato
// pro mais caro, e para no primeiro que passa do piso. Só vale sozinho — junto com
...
async function buscaFreteAbaixoDoPiso(atual) {
  ...
  for (let off = 0; off < CONFIG.OFFSET_CAP && !passouDoPiso; off += CONFIG.PAGE_SIZE) {
    const p = new URLSearchParams();
    p.set('seller_id', state.sellerId);
    p.set('status', state.status === 'all' ? 'all' : state.status);
    p.set('labels', 'with_free_shipping');
    p.set('order', 'price_asc');
    p.set('limit', String(CONFIG.PAGE_SIZE));
    p.set('offset', String(off));
    const resp = await proxyGet('/api/fetch-ads?' + p.toString());
    const ids = idsDaResposta(resp || {});
    if (!ids.length) break;
    const itens = await hydrate(ids);
    vistos += itens.length;
    itens.forEach((it) => {
      if (Number(it.price) >= PROBLEM_RULES.freeShippingFloor) { passouDoPiso = true; return; }
      if (isCheapFreeShipping(it)) achados.push(it);
    });
    if (host && vivo() && !passouDoPiso) host.innerHTML = loadingHtml(`Procurando os anúncios com frete grátis mais baratos… ${vistos}`);
    if (ids.length < CONFIG.PAGE_SIZE) break;
  }
...
function passaNosRecortes(it) {
  if (state.discountOnly && !(discountPct(it) > 0)) return false;
  if (state.freeShipUnder && !isCheapFreeShipping(it)) return false;
  return true;
}
function idsDaResposta(resp) {
  const results = Array.isArray(resp.results) ? resp.results : [];
  return results.map((it) => (typeof it === 'string' ? it : (it && (it.id || it.item_id)))).filter(Boolean);
}
// Roda as tarefas com no máximo N em voo — conta de 761 anúncios são ~55 chamadas,
// e em fila indiana isso vira meio minuto de espera olhando spinner.
async function emParalelo(tarefas, limite, aoConcluir) {
...
// `atual` diz se esta varredura ainda interessa: trocar de filtro no meio dela abandona
// a carga, e uma varredura órfã escrevendo progresso apagava a lista que já tinha sido
// renderizada pela carga nova. O trabalho feito continua indo pro cache — só a tela para.
async function scanAccount(atual) {
  const chave = scanKey();
  if (state.scan && state.scan.chave === chave) return state.scan;
  const host = $('#tableHost');
  const vivo = () => (typeof atual === 'function' ? atual() : true);

  // Caminho curto: a ML entrega esse recorte pronto (2 a 3 chamadas em vez de ~55)
  if (podeFiltrarFreteNoServidor()) {
    try {
      const r = await buscaFreteAbaixoDoPiso(atual);
      state.scan = { chave: chave, itens: r.itens, varridos: r.varridos, total: r.varridos, truncado: false, parcial: false, viaServidor: true };
      return state.scan;
    } catch (e) {
  ...
  const progresso = (feitos, alvo) => {
    if (host && vivo()) host.innerHTML = loadingHtml(`Procurando na sua conta inteira… ${Math.min(feitos, alvo)} de ${alvo} anúncios`);
  };

  // 1) primeira página dá o total — só com ele dá pra paralelizar o resto
  const primeira = await mlGet(buildListUrl(0));
  const idsPrimeira = idsDaResposta(primeira);
  const total = (primeira.paging && typeof primeira.paging.total === 'number') ? primeira.paging.total : idsPrimeira.length;
  const alvo = Math.min(total, CONFIG.OFFSET_CAP);
  progresso(0, alvo);

  // 2) demais páginas de IDs, em paralelo
  const offsets = [];
  for (let off = CONFIG.PAGE_SIZE; off < alvo; off += CONFIG.PAGE_SIZE) offsets.push(off);
  let falhas = 0;
  const outras = await emParalelo(
    offsets.map((off) => () => mlGet(buildListUrl(off)).then(idsDaResposta).catch(() => { falhas++; return []; })),
    CONFIG.SCAN_CONCURRENCY
  );
  const todosIds = idsPrimeira.concat.apply(idsPrimeira, outras).slice(0, alvo);

  // 3) multiget em paralelo, com o progresso contando anúncios já lidos
  const lotes = [];
  for (let i = 0; i < todosIds.length; i += CONFIG.HYDRATE_CHUNK) lotes.push(todosIds.slice(i, i + CONFIG.HYDRATE_CHUNK));
  let lidos = 0;
  const partes = await emParalelo(
    lotes.map((lote) => () => hydrate(lote).catch(() => { falhas++; return []; })),
    CONFIG.SCAN_CONCURRENCY,
    () => { lidos += CONFIG.HYDRATE_CHUNK; progresso(lidos, alvo); }
  );

  const achados = [];
  let lidosDeVerdade = 0;
  partes.forEach((itens) => (itens || []).forEach((it) => {
    lidosDeVerdade++;
    if (passaNosRecortes(it)) achados.push(it);
  }));
  // Varredura com buraco (429, proxy oscilando) NÃO pode virar cache: "0 com desconto"
  // por causa de chamada que falhou é pior do que não responder.
  state.scan = {
    chave: falhas ? null : chave,
    itens: achados, varridos: lidosDeVerdade, total: total,
    truncado: total > CONFIG.OFFSET_CAP, parcial: falhas > 0
  };
  return state.scan;
}
// Qualquer coisa que mude o conjunto de anúncios invalida a varredura guardada
function invalidarScan() { state.scan = null; }
...
function computeBadges(item) {
  ...
  if (faltando.length) {
    ...
    // A coluna de sinais tem largura fixa e nome de atributo do ML chega a 30 letras
    // ("Temperatura máxima suportada"): dois deles juntos passavam por cima do botão
    // Analisar. Mostra quantos couberem e o resto vira "+N" — a lista inteira fica na dica.
    const montaTexto = (qtd) => {
      const resto = faltando.length - qtd;
      return `Ficha: falta ${nomes.slice(0, qtd).join(', ')}` + (resto > 0 ? ` +${resto}` : '');
    };
    let texto = '';
    if (nomes.length) {
      texto = montaTexto(Math.min(2, nomes.length));
      if (texto.length > CONFIG.FICHA_TEXTO_MAX && nomes.length > 1) texto = montaTexto(1);
    }
    if (!texto || texto.length > CONFIG.FICHA_TEXTO_MAX) {
      texto = faltando.length === 1 ? 'Ficha técnica: falta 1 campo' : `Ficha técnica: faltam ${faltando.length} campos`;
    }
  ...
  // "Sem frete grátis" só a partir do piso do ML (R$ 79): abaixo disso a plataforma
  // não obriga nem subsidia frete grátis, então o vendedor está certo em não dar —
...
BADGE_TITLES['Sem frete grátis'] = 'A partir de R$ ' + PROBLEM_RULES.freeShippingFloor + ' o Mercado Livre destaca quem oferece frete grátis, e parte do custo é subsidiada. Abaixo desse valor este sinal nem aparece.';
...
function unionBadges(items, agg) {
  ...
  // Ficha técnica: cada variação tem campos diferentes, então a união rendia dois ou
  // três "Ficha: falta ..." empilhados na mesma linha. Na linha-família o que decide é
  // QUANTAS variações estão incompletas — os campos aparecem ao expandir.
  const fichas = arr.filter((b) => /^Ficha/.test(b.text));
  if (multi && fichas.length > 1) {
    const incompletas = items.filter((it) => {
      const f = state.fichaMap[it.id];
      return !!(f && f.missing && f.missing.length);
    }).length;
    const primeira = arr.indexOf(fichas[0]);
    arr = arr.filter((b) => !/^Ficha/.test(b.text));
    arr.splice(primeira, 0, { text: `Ficha incompleta em ${incompletas} variações`, cls: 'orange' });
  }
  return arr;
}
...
function discountBannerHtml() {
  ...
  return `<div class="banner info" style="margin:12px 12px 0"><span>ℹ️</span><span>Mostrando só: <b>Com desconto</b> (na sua conta inteira) — <button type="button" class="chip-clear-link" id="discountClearBtn">voltar para a minha lista</button></span></div>`;
}
...
function freeShipBannerHtml() {
  ...
  return `<div class="banner info" style="margin:12px 12px 0"><span>ℹ️</span><span>Mostrando só: <b>Frete grátis abaixo de R$ ${PROBLEM_RULES.freeShippingFloor}</b> (na sua conta inteira) — nesses anúncios o frete sai do seu bolso — <button type="button" class="chip-clear-link" id="freeShipClearBtn">voltar para a minha lista</button></span></div>`;
}
...
function pagerHtml() {
  ...
  // Recorte de conta inteira: o total já é o do recorte, então o pager conta como
  // qualquer outra lista — só diz de que recorte se trata.
  const cuts = [];
  ...
  const varridos = state.scan ? Math.min(state.scan.varridos, CONFIG.OFFSET_CAP) : 0;
  // Filtro feito pela própria ML não "varre" nada: dizer um número de anúncios
  // percorridos ali seria mentira (só os com frete grátis passaram pela conta).
  const alcance = (state.scan && state.scan.viaServidor)
    ? 'procurados na sua conta inteira'
    : `procurados em ${varridos} anúncios da sua conta`;
  const info = cuts.length
    ? `Mostrando <b>${shownFrom}–${shownTo}</b> de <b>${total}</b> anúncios ${cuts.join(' e ')}${grouped} <span class="pager-scope">— ${alcance}</span>`
...
async function loadPage() {
  const host = $('#tableHost');
  // Carga tem geração: a varredura de conta inteira leva alguns segundos, e trocar de
  // filtro no meio dela dispara outra carga. Sem isso, a resposta da carga ABANDONADA
  // chega depois e pinta a tela com números de um filtro que não está mais ligado.
  const geracao = ++state.carga;
  const desatualizada = () => geracao !== state.carga;
  state.loading = true;
  ...
  try {
    ...
    // Recorte de conta inteira ligado: varre tudo uma vez e pagina sobre o resultado
    if (quickFilterAtivo()) {
      ...
      // Conta maior que o teto de navegação da ML: dizer onde a busca parou, senão o
      // vendedor lê o resultado como se fosse a conta inteira.
      if (scan.parcial) {
        setBanner('warn', `Parte dos seus anúncios não respondeu agora — este recorte saiu de ${scan.varridos} anúncios, não da conta inteira. Clique no filtro de novo em alguns segundos para refazer a busca.`);
      } else if (scan.truncado) {
        setBanner('info', `Procuramos nos primeiros ${CONFIG.OFFSET_CAP} anúncios (o Mercado Livre limita a navegação a esse número). Use os filtros acima para varrer o resto da conta.`);
      }
      loadAttrNames();
...
function renderError(err) {
  ...
  if (err && err.status === 401) {
    // Estado explícito em vez de tela em branco/antiga: a conexão do ML é a mesma
    // que a análise usa, então enquanto ela não voltar NADA aqui funciona. O 2º
    // botão evita o "e agora?" depois de reconectar na outra aba.
    host.innerHTML = `<div class="state">
      <span class="emoji">🔌</span>
      <h3>Sua conta do Mercado Livre precisa ser reconectada</h3>
      <p>A conexão com o Mercado Livre expirou. Reconecte em <b>Minha Conta → Adicionar Conta</b> — leva alguns segundos e suas outras ferramentas voltam junto.</p>
      <button class="btn-retry" id="reconnectBtn">Reconectar conta</button>
      <button class="btn-retry" id="reconnectedBtn" style="margin-left:8px;background:transparent;color:var(--blue)">Já reconectei</button>
    </div>`;
    const rc = $('#reconnectBtn');
    if (rc) rc.addEventListener('click', () => window.open(CONFIG.RECONNECT_URL, '_blank'));
    const rd = $('#reconnectedBtn');
    if (rd) rd.addEventListener('click', () => location.reload());
    return;
  }
  if (err && err.status === 429) {
    // Excesso de consultas seguidas (varredura de conta inteira, muitos filtros em
    // sequência). Não é conta desconectada nem erro do app: em um minuto volta.
    host.innerHTML = `<div class="state">
      <span class="emoji">⏳</span>
      <h3>Muitas consultas ao Mercado Livre de uma vez</h3>
      <p>Fizemos consultas demais em pouco tempo e o Mercado Livre pediu uma pausa. Espere cerca de um minuto e tente de novo — nada foi perdido.</p>
      <button class="btn-retry" id="retryBtn">Tentar de novo</button>
    </div>`;
    const rb429 = $('#retryBtn');
    // sem sellerId o boot nem chegou ao fim: recarregar é o caminho honesto
    if (rb429) rb429.addEventListener('click', () => { if (state.sellerId) loadPage(); else location.reload(); });
    return;
  }
  host.innerHTML = `<div class="state">
    <span class="emoji">😕</span>
...
function exportCsv() {
  ...
  if (quickFilterAtivo() && state.scan && state.scan.chave === scanKey()) {
    ...
    setBanner('info', 'Planilha baixada com os ' + items.length + ' anúncios do recorte, procurados na conta inteira.');
    return;
  }
  exportAllCsv();
}
...
async function boot() {
  ...
  try {
    ...
    // 401 = conexão do Mercado Livre vencida. Aqui o fluxo legado NÃO ajuda: o
    // analyzer usa o MESMO token e vai falhar do mesmo jeito, e o painel sumindo
    // parece que a tela mudou sozinha. Então o painel FICA, dizendo o que houve e
    // com o botão de reconectar. Qualquer outra falha (proxy fora, erro
    // inesperado) continua recolhendo pro fluxo antigo.
    // 429 = consultas demais em pouco tempo (a varredura de conta inteira usa várias
    // por vez). Cair no fluxo legado aqui seria pior: a tela "muda sozinha" por causa
    // de uma pausa que passa em um minuto. O painel fica, com o estado explicado.
    if (err && (err.status === 401 || err.status === 429)) {
      const chips = document.getElementById('chipsCard');
      const filters = document.getElementById('filtersCard');
      if (chips) chips.hidden = true;      // controles não fazem sentido sem conta conectada
      if (filters) filters.hidden = true;
      const sub = document.getElementById('subtitle');
      if (sub) {
        sub.textContent = err.status === 401
