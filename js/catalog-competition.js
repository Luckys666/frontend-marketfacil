...
// --- Global Variables ---
  ...
  async function fetchItemDetails(itemId) {
    ...
    // ML's /products/.../items retorna `original_price` quase sempre null mesmo com desconto vigente,
    // então o scraper.price (riscado <s>) é a fonte mais confiável pro preço cheio.
    const _sdPrice = parseFloat(sD.price);
    const _sdDiscounted = parseFloat(sD.price_discounted);
    const _apiOriginal = parseFloat(aD.original_price);
    const _apiPrice = (typeof aD.price === 'number') ? aD.price : parseFloat(aD.price);
    const pDF = Number.isFinite(_apiPrice) ? _apiPrice
              : Number.isFinite(_sdDiscounted) ? _sdDiscounted
              : 0;
    const pF  = Number.isFinite(_sdPrice) ? _sdPrice
              : Number.isFinite(_apiOriginal) ? _apiOriginal
              : pDF;
