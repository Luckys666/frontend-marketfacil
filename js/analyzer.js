...
function exibirTendenciaVisitas(visitsData, containerId = "visitsTrend") {
    ...
    const prev15 = results.slice(Math.max(0, len - 30), Math.max(0, len - 15));
    const last30 = results.slice(Math.max(0, len - 30));
    const prev30 = results.slice(Math.max(0, len - 60), Math.max(0, len - 30));
    ...
    let percentChange7 = calcPct(total7, totalPrev7);
    const percentChange15 = prev15.length >= 7 ? calcPct(total15, totalPrev15) : null;
    const percentChange30 = prev30.length >= 15 ? calcPct(total30, totalPrev30) : null;
