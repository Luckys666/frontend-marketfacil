...
function exibirQualidadeFicha(qualidade, categoryAttributes, containerId = "qualidadeFicha") {
    ...
    // (atributo de família: o endpoint que edita a família inteira sem quebrar o
    // vínculo é bloqueado pro nosso app, e mexer pelo item pode tirar o anúncio do
    // grupo de variações). Marcar no próprio chip evita a pergunta "por que esse
    // campo não aparece pra editar?".
    const detalheAtual = window.currentAnalysisState?.detail;
    const defPorId = {};
    ...
    const listaFicha = faltando.length ? `
        ...
        <div style="display:flex; flex-wrap:wrap; gap:6px;">
            ${faltando.map((c) => {
                const fora = soNoML(c);
                const selo = fora
                    ? `<span style="font-size:0.68rem; font-weight:600; opacity:.75; margin-left:6px;" title="Vale para todas as variações do produto — só dá para preencher direto no Mercado Livre">só no ML</span>`
                    : '';
        ...
        ${marcados ? `<p class="text-small" style="margin:8px 0 0; color:var(--text-muted);">${marcados === 1 ? 'O campo marcado vale' : 'Os campos marcados valem'} para todas as variações — edite no Mercado Livre.</p>` : ''}` : '';
