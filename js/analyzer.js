...
function exibirCompatibilidades(veredito, containerId = 'compatibilidades') {
    ...
    const categoriaDoVeredito = veredito.categoria || {};
    const bloqueio = veredito.ja_e_universal === true
        ...
        ? 'Este anúncio está marcado como "serve em qualquer veículo". Enquanto essa marcação estiver ativa, o Mercado Livre não aceita adicionar veículos a ele — e o app não desfaz marcação que você fez. Para mudar, use o Gerenciador de Compatibilidades no próprio Mercado Livre.'
        : categoriaDoVeredito.exige_posicao === true
            ? 'Nesta categoria o Mercado Livre também exige a posição da peça (dianteira ou traseira, esquerda ou direita), e isso ainda não dá para fazer pelo app. Para indicar os veículos deste anúncio, use o Gerenciador de Compatibilidades no próprio Mercado Livre.'
            : null;
    const blocoBloqueio = bloqueio
        ? `<div class="mf-compat-bloqueio" style="margin-top:10px; padding:8px 10px; background:var(--bg-subtle,#f1f5f9); border-left:3px solid var(--text-muted);">
               <p class="text-small">${escapeHtml(bloqueio)}</p>
           </div>`
        : '';
