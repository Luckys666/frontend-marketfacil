...
function MF_renderEscadaCompat(state) {
    ...
    const varios = escopoConhecido && alc.itens.length > 1;
        ...
        : alc.erro
            ...
            : varios
                ? `<div class="mf-compat-alcance" style="margin-top:10px; padding:8px 10px; background:var(--bg-subtle,#f1f5f9); border-left:3px solid var(--text-muted);">
                       <p class="text-small" style="font-weight:600;">⚠️ Isto vale para ${alc.itens.length} anúncios:</p>
...
async function MF_compatCarregarAlcance(state) {
    ...
    try {
        ...
        if (!res.ok || !Array.isArray(dados.itens)) {
            ...
            esc.alcance = { carregando: false, erro: null, itens: dados.itens };
        }
    } catch (e) {
        esc.alcance = { carregando: false, erro: 'nao_deu_pra_consultar', itens: null };
    }
