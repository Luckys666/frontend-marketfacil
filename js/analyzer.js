...
async function startAccountScan() {
    ...
    const filterVal = document.getElementById('tagFilter').value;
    ...
    try {
        const [accessToken, userId] = await Promise.all([fetchAccessToken(), fetchUserIdForScraping()]);

        if (!accessToken || !userId) {
            throw new Error('Não foi possível obter credenciais (Token ou User ID). Verifique sua conexão.');
        }

        if (statusText) statusText.textContent = 'Buscando lista de anúncios...';

        // 1. Fetch ALL Item IDs
        const allIds = await fetchAllUserItemIds(userId, accessToken, (progress) => {
            if (statusText) statusText.textContent = `Mapeando anúncios: ${progress.loaded} encontrados...`;
        });

        const totalItems = allIds.length;
        if (statusText) statusText.textContent = `Analisando ${totalItems} anúncios...`;
        if (countText) countText.textContent = `0 detectados (filtro: ${filterVal})`;

        // 2. Process in Batches (Fetch Details & Filter)
        const BATCH_SIZE = 50;
        ...
        for (let i = 0; i < totalItems; i += BATCH_SIZE) {
            ...
            if (statusText) statusText.textContent = `Analisando: ${processedCount}/${totalItems} (${percent}%)`;

            try {
                // Fetch Details for Batch
                const details = await fetchDetailsForScanner(batchIds, accessToken);
                ...
                const matches = details.filter(item => {
                    ...
                    return tags.includes(filterVal);
                });

                // Render Matches
                if (matches.length > 0) {
                    renderScannerBatch(matches);
                    matchesCount += matches.length;
                    if (countText) countText.textContent = `${matchesCount} encontrados`;
                }
        ...
        if (statusText) statusText.textContent = 'Scanner concluído!';
        if (scanBtn) scanBtn.disabled = false;

        if (matchesCount === 0) {
            resultsContainer.innerHTML = `
                <div class="ana-card" style="grid-column: 1 / -1; text-align: center; padding: 40px;">
                    <span style="font-size: 2rem;">🤷‍♂️</span>
                    <p>Nenhum anúncio encontrado com o filtro "<strong>${filterVal}</strong>".</p>
                </div>
            `;
        }
...
async function fetchAllUserItemIds(userId, accessToken, onProgress) {
    let allIds = [];
    let offset = 0;
    let total = 1; // Assume at least 1 to start
    const LIMIT = 50;

    // Direct MLB API Call for Search (Assuming CORS allows or Proxy fallback needed)
    // NOTE: If CORS fails, we might need a proxy route. Trying direct first as per standard integrations.
    const SEARCH_API = `https://api.mercadolibre.com/users/${userId}/items/search`;

    while (offset < total) {
        const url = `${SEARCH_API}?search_type=scan&limit=${LIMIT}&offset=${offset}`;
        try {
            ...
            if (onProgress) onProgress({ loaded: allIds.length, total: total });

            // Avoid rate limits
            await new Promise(r => setTimeout(r, 100));

        } catch (e) {
            console.error('Erro ao buscar IDs:', e);
            break; // Stop on error
        }
    }
    return allIds;
...
async function fetchDetailsForScanner(ids, accessToken) {
    // Reuses existing batch fetch logic if available, or direct calls
    // The existing fetchItemDetails handles commas.
    // MAX URL length can be an issue, but 50 IDs is usually fine.
    try {
        const data = await fetchItemDetails(ids, accessToken); // Returns array of { code, body }

        // Transform to simple array of objects
        return data.map(d => (d.code === 200 ? d.body : null)).filter(i => i !== null);
    } catch (e) {
        console.warn('Falha ao buscar detalhes do lote:', e);
        return [];
    }
}

function renderScannerBatch(items) {
    const container = document.getElementById('scannerResults');
    const initial = container.querySelector('.initial-placeholder');
    ...
    items.forEach(item => {
        ...
        let tagsHtml = '';
        tags.forEach(t => {
            if (TAGS_NEGATIVAS.has(t)) {
                tagsHtml += `<span class="status-badge error" style="margin-right:4px; margin-bottom:4px; font-size:0.7rem;">${t}</span>`;
            } else if (t === 'good_quality_picture' || t === 'good_quality_thumbnail') {
                // Skip 'good' tags in default view to avoid clutter, unless filtering for them?
                // Let's show them but small
            } else {
                tagsHtml += `<span class="status-badge muted" style="margin-right:4px; margin-bottom:4px; font-size:0.7rem;">${t}</span>`;
            }
        });

        card.innerHTML = `
            <div style="display:flex; gap:10px; margin-bottom:10px;">
                <img src="${thumb}" style="width:60px; height:60px; object-fit:cover; border-radius:4px; border:1px solid #f1f5f9;">
                <div>
                    <a href="${item.permalink}" target="_blank" class="text-value" style="font-size:0.9rem; line-height:1.2; text-decoration:none; color:#1e293b; display:block; margin-bottom:4px;">${item.title}</a>
                    <span style="font-weight:600; color:#10b981; font-size:0.9rem;">${price}</span>
                </div>
            </div>
            ...
            <div style="display:flex; justify-content:flex-end;">
                 <button onclick="analisarAnuncio('${item.id}')" style="background:none; border:none; color:#3b82f6; cursor:pointer; font-weight:600; font-size:0.85rem;">Analisar Detalhes ➔</button>
            </div>
        `;
