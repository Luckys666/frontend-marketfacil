...
function exibirAtributosCategoria(categoryAttributes, adAttributes, containerId = "categoryAttributes") {
    ...
    if (!Array.isArray(categoryAttributes) || stringAttributes.length === 0) {
        ...
        stringAttributes.forEach(catAttr => {
            ...
            } else if (motivo === 'nomevariacao') {
                // Fica na tela de propósito, em faixa própria. Travar só resolve metade: o
                // vendedor pode renomear pelo ML e perder a exposição do mesmo jeito, e aí
                // some da nossa tela sem ele nunca ter visto o preço. Fora das etapas para
                // não entrar em placar nenhum — a contagem do cabeçalho é mfCampoEditavel.
                nomeVariacaoAttrs.push({ catAttr, adValue });
            } else if (motivo) {
        ...
        // O que dá nome à variação: mostrado, nunca editável, com o preço escrito.
        // O aviso vale igual para quem for mexer no Mercado Livre — é a única coisa que a
        // gente consegue fazer por esse caso, já que fora do app não dá pra impedir.
        let avisoNomeVariacao = '';
        if (nomeVariacaoAttrs.length > 0) {
            const itens = nomeVariacaoAttrs
                .map(({ catAttr, adValue }) => `<strong>${escapeHtml(catAttr.name)}:</strong> ${escapeHtml(adValue)}`)
                .join(' &nbsp;·&nbsp; ');
            avisoNomeVariacao = `
                <div style="background:var(--bg-subtle,#f1f5f9); border:1px solid var(--border,#e2e8f0); padding:10px 12px; border-radius:6px; margin-top:4px; display:flex; gap:10px; align-items:flex-start;">
                    <span style="font-size:1rem; line-height:1.3;">🔒</span>
                    <div style="flex:1; min-width:0;">
                        <div style="font-size:0.82rem; color:var(--text); word-break:break-word;">${itens}</div>
                        <div style="font-size:0.75rem; color:var(--text-secondary); margin-top:2px;">Dá nome a esta variação. Renomear — aqui ou no Mercado Livre — muda o link do anúncio, e ele perde a exposição que já tinha.</div>
                    </div>
                </div>`;
        }
