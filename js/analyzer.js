...
const MF_COMPAT_ERROS = {
    ...
    // Este anúncio já está marcado como "serve em qualquer veículo": enquanto a marcação
    // estiver lá, a ML recusa veículo novo — e o app não remove marcação de ninguém (§7.2).
    // Sem link: a URL do Gerenciador não foi levantada (§10.6), então o texto descreve o
    // caminho em vez de fingir que há atalho.
    ja_e_universal: 'Este anúncio está marcado como "serve em qualquer veículo", e por isso o Mercado Livre não aceita adicionar veículos a ele. Para mudar, use o Gerenciador de Compatibilidades no próprio Mercado Livre.',
    // O teto de 200 da ML é de PRODUTOS resolvidos, não dos veículos que o vendedor
    // escolheu: um "usar o modelo inteiro" vira dezenas de produtos sozinho. Sem essa
    // frase, ele olha "3 veículos escolhidos", lê "limite de 200" e conclui que o app
    // quebrou. Diz também o que fazer, não só o que deu errado.
    passou_de_200_produtos: 'O Mercado Livre não conta os veículos que você escolheu, e sim cada versão e ano que eles representam — por isso "usar o modelo inteiro" pode passar do limite de 200 sozinho. Escolha anos específicos, ou grave em duas etapas.',
...
/**
 * Estados em que a gravação já nasce recusada pela ML (v480). Texto único, usado no card
 * (que esconde os botões) e nas funções de gravação (que recusam a chamada).
 */
const MF_COMPAT_BLOQUEIOS = {
    // A palavra "universal" não aparece: na tela isso sempre se chama "serve em qualquer
    // veículo", que é como o próprio botão do card chama.
    ja_e_universal: 'Este anúncio está marcado como "serve em qualquer veículo". Enquanto essa marcação estiver ativa, o Mercado Livre não aceita adicionar veículos a ele — e o app não desfaz marcação que você fez. Para mudar, use o Gerenciador de Compatibilidades no próprio Mercado Livre.',
    exige_posicao: 'Nesta categoria o Mercado Livre também exige a posição da peça (dianteira ou traseira, esquerda ou direita), e isso ainda não dá para fazer pelo app. Para indicar os veículos deste anúncio, use o Gerenciador de Compatibilidades no próprio Mercado Livre.',
};

/**
 * Por que esta tela NÃO pode gravar compatibilidade agora — ou `null`.
 *
 * Fonte ÚNICA, de propósito: o card usa isto para esconder os botões e as duas funções de
 * gravação usam para recusar a chamada. Quando os dois lados calculam a mesma regra
 * separado, um dia eles discordam — e o que sobra é sempre o botão clicável, que é o pior
 * dos dois erros.
 *
 * `null` em qualquer campo de `categoria` não decide nada: só `true` explícito bloqueia.
 */
function MF_compatMotivoDeBloqueio(veredito) {
    if (!veredito) return null;
    if (veredito.ja_e_universal === true) return 'ja_e_universal';
    if (veredito.categoria && veredito.categoria.exige_posicao === true) return 'exige_posicao';
    return null;
}
...
function exibirCompatibilidades(veredito, containerId = 'compatibilidades') {
    ...
    // v480 — dois estados em que a gravação já nasce recusada pela ML. Aqui a tela não pode
    // oferecer botão nenhum: o pior erro que esta feature pode cometer é convidar o vendedor
    // a montar uma lista de 12 veículos para ele descobrir no fim que nada disso ia funcionar.
    // `null` em qualquer campo de `categoria` é "não sei" e NÃO decide nada — sem o dado, a
    // tela segue como se ele não existisse.
    const bloqueio = MF_compatMotivoDeBloqueio(veredito);
    const blocoBloqueio = bloqueio
        ? `<div class="mf-compat-bloqueio" style="margin-top:10px; padding:8px 10px; background:var(--bg-subtle,#f1f5f9); border-left:3px solid var(--text-muted);">
               <p class="text-small">${escapeHtml(MF_COMPAT_BLOQUEIOS[bloqueio])}</p>
           </div>`
        : '';
    // Com bloqueio, NENHUM remédio é oferecido: copiar e universal também gravam
    // compatibilidade, então cairiam na mesma recusa.
    const acoes = bloqueio ? '' : (botaoUniversal + botaoCopiar + botaoEscada);
...
window.mfCompatUniversal = async function () {
    ...
    // Mesma guarda da escada, mesma fonte de verdade: o card pode estar velho na tela.
    const bloqueadoAgora = MF_compatMotivoDeBloqueio(state.compatData);
    if (bloqueadoAgora) {
        exibirCompatibilidades(state.compatData, `compatibilidades${state.containerIdSuffix || ''}`);
        MF_erroAtalho('compat', MF_COMPAT_BLOQUEIOS[bloqueadoAgora]);
        return;
    }
...
function MF_renderEscadaCompat(state) {
    ...
    // Guarda §7.4 do COMPAT-SPEC: quando o anúncio tem user-product, a gravação atinge
    // TODOS os anúncios que usam aquele produto. Medido na conta real (15/08/2026): o
    // user-product MLBU1115911717 sustenta 2 anúncios. Gravar sem essa lista à vista é
    // surpresa em anúncio que vende. O escopo é perguntado ao proxy quando a escada abre.
    const alc = esc.alcance;
    const escopoConhecido = !!(alc && !alc.carregando && !alc.erro && Array.isArray(alc.itens));
    // A contagem vem do `total` do proxy, não do tamanho da lista: numa família maior que a
    // página da ML a lista é um pedaço, e é o total que diz a verdade.
        ...
        : alc.erro
            ...
            : varios
                ? `<div class="mf-compat-alcance" style="margin-top:10px; padding:8px 10px; background:var(--bg-subtle,#f1f5f9); border-left:3px solid var(--text-muted);">
                       ...
                       ${alc.itens.map((i) => `<div class="text-small">• ${escapeHtml(i.title || i.id || '')}</div>`).join('')}
                       ${restantes > 0 ? `<div class="text-small" style="color:var(--text-muted);">e mais ${restantes} que não couberam nesta lista.</div>` : ''}
                   </div>`
                : '';
    ...
    // v480: `categoria.aceita_nota === false` esconde o campo. Oferecer um campo que a ML
    // vai recusar é pior que não ter campo — o vendedor escreve o aviso que protege o
    // comprador e ele é jogado fora sem ninguém contar. `null` não decide nada.
    const nota = typeof esc.nota === 'string' ? esc.nota : '';
    const blocoNota = (esc.selecoes.length && MF_compatAceitaNota(state)) ? `
...
/**
 * A categoria deste anúncio aceita nota? Só `false` explícito nega — `null` é "não sei" e
 * não decide nada (v480). Vale para desenhar o campo E para montar o corpo da gravação: se
 * o vendedor digitou antes do veredito chegar, a nota não pode viajar assim mesmo, porque
 * a ML recusaria a gravação inteira por causa dela.
 */
function MF_compatAceitaNota(state) {
    const cat = state && state.compatData && state.compatData.categoria;
    return !(cat && cat.aceita_nota === false);
}
...
window.mfCompatAbrirEscada = async function () {
    ...
    if (!state.escadaCompat) {
        state.escadaCompat = { nivel: 'marca', opcoes: [], carregando: false, erro: null, marca: null, modelo: null, selecoes: [], alcance: null };
    }
    // O escopo da escrita (guarda §7.4) roda EM PARALELO com o primeiro nível: são duas
    // perguntas independentes, e segurar a escada esperando o escopo só deixaria a tela
    // lenta. Quem espera é o botão de gravar, não o vendedor navegando.
    MF_compatCarregarAlcance(state);
...
/**
 * Pergunta ao proxy QUAIS anúncios a gravação vai atingir (COMPAT-SPEC §7.4). A regra de
 * quem é atingido mora no proxy — aqui só se desenha a lista que ele mandar.
 * Falha nunca vira "atinge só este" (feedback_falha_nunca_vira_zero): vira `erro`, e o
 * botão de gravar passa a dizer que é sem conferir.
 */
...
window.mfCompatConferirAlcanceDeNovo = function () {
    const state = window.currentAnalysisState;
    if (!state || !state.escadaCompat) return;
    state.escadaCompat.alcance = null;
    return MF_compatCarregarAlcance(state);
};
...
window.mfCompatGravarVeiculos = async function () {
    ...
    // A escada pode estar ABERTA com um veredito velho: o card novo nasce sem os botões, mas
    // estado envelhece — quem já estava com o painel aberto quando o veredito virou continua
    // com um botão que a tela já sabe que vai falhar. Recusa antes de gastar a chamada, e
    // redesenha o card ANTES da mensagem (o redesenho apagaria o aviso se viesse depois), pra
    // tela velha se consertar sozinha em vez de deixar o vendedor clicando num botão morto.
    const bloqueadoAgora = MF_compatMotivoDeBloqueio(state.compatData);
    if (bloqueadoAgora) {
        exibirCompatibilidades(state.compatData, `compatibilidades${state.containerIdSuffix || ''}`);
        MF_erroAtalho('compat', MF_COMPAT_BLOQUEIOS[bloqueadoAgora]);
    ...
    try {
        ...
        const nota = (MF_compatAceitaNota(state) && typeof esc.nota === 'string') ? esc.nota.trim() : '';
