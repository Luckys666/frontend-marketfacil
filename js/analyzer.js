 ...
 *   - PARENT_PK/FAMILY em anúncio de família (user_product_id): mexer aqui NÃO é só
 *     um 403 da API — os atributos PARENT_PK são o que agrupa as variações. A doc do
 *     ML (User Products) diz, com todas as letras, que "modificar os atributos dos
 *     itens pode fazer com que saiam da família atual, por exemplo, ao alterar a marca,
 *     modelo". O editor oficial de família (POST /user-products-families/{id}/tasks)
 *     só aceita CHILD_PK e atributos custom, e exige mandar TODOS os UPs da família.
 *     Ou seja: pedir "preencha a Marca" num item de família é pedir pro vendedor
 *     quebrar o agrupamento do produto.
 */
...
function exibirAtributosCategoria(categoryAttributes, adAttributes, containerId = "categoryAttributes") {
    ...
    if (!Array.isArray(categoryAttributes) || stringAttributes.length === 0) {
        ...
        const foraPorMotivo = { sistema: [], familia: [] };
        (Array.isArray(categoryAttributes) ? categoryAttributes : []).forEach((c) => {
            const m = mfMotivoNaoEditavel(c, detail);
            if (m === 'sistema' || m === 'familia') foraPorMotivo[m].push(c.name || c.id);
        });
        const foraDaConta = foraPorMotivo.sistema.concat(foraPorMotivo.familia);
        // Uma linha só. O detalhe que importa — mexer nos campos da família tira o
        // anúncio do grupo de variações (regra do ML) — fica no tooltip.
        const dicaFora = foraPorMotivo.familia.length
            ? `Campos do produto (${foraPorMotivo.familia.join(', ')}) são iguais em todas as variações: mudar aqui tira este anúncio do grupo. Os outros o Mercado Livre preenche sozinho.`
            : foraDaConta.join(', ');
        const notaForaDaConta = foraDaConta.length
            ? `<p class="text-small" style="margin:2px 0 0; color:var(--text-muted); cursor:help;" title="${escapeHtml(dicaFora)}">${foraDaConta.length} campos ${foraPorMotivo.familia.length ? 'vêm do produto ou do Mercado Livre' : 'são preenchidos pelo Mercado Livre'} e não contam aqui.</p>`
            : '';
